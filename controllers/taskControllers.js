...
export const getTask = async (req, res) => {
    ...
    // Admin gets all, user gets only assigned
    let tasks;
    ...
    const countFilter = req.user.role === "admin" ? {} : { assignedTo: req.user._id };
...
export const createTask = async (req, res) => {
    try {
        ...
        if (!Array.isArray(assignedTo)) {
            return res
            .status(400)
            .json({message: "assignTo must be an array of user ID's"});
        }
...
export const updateTaskChecklist = async (req, res) => {
  try {
    ...
    task.todoChecklist = todoChecklist; // Replace checklist
...
//@route GET /api/tasks/dashbard-data
//@access PRIVATE
export const getDashboardData = async (req, res) => {
  try {
    ...
    const tasksPriorityLevels = taskPriorities.reduce((acc, priority) => {
      acc[priority] =
...
//@route GET /api/tasks/user-dashbard-data
//@access PRIVATE
